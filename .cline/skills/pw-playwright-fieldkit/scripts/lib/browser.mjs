...
export async function launch(opts = {}) {
  ...
  if (opts.storageState && !existsSync(opts.storageState)) {
    log.err(`Auth state file not found: ${opts.storageState}. Create it first with:  node .cline/skills/pw-playwright-fieldkit/scripts/save-auth.mjs <loginUrl> --headed --out ${opts.storageState}`);
    process.exit(2);
  }
