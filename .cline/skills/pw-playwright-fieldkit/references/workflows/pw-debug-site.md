# /pw-debug-site — find and reproduce bugs by using the site
...
Step 2C. The user does not need to type `/pw-debug-site`.

## Step 2A — If you must find the bug: crawl
```
node .cline/skills/pw-playwright-fieldkit/scripts/crawl.mjs <START_URL> --depth 2 --max-pages 40 --out report/debug
```
...
```
node .cline/skills/pw-playwright-fieldkit/scripts/inspect.mjs <URL> --wait 2000 --out report/debug
```
...
```
node .cline/skills/pw-playwright-fieldkit/scripts/inspect.mjs <URL> --click "text=Load more" --click "#save" --wait 2000
```
...
1. Inspect the **source page**:
   ```
   node .cline/skills/pw-playwright-fieldkit/scripts/inspect.mjs <SOURCE_URL> --wait 1000 --out report/debug-links
   ```
...
4. Verify normal same-tab behavior by clicking the exact selector printed in the
   report:
   ```
   node .cline/skills/pw-playwright-fieldkit/scripts/inspect.mjs <SOURCE_URL> --click '<SELECTOR>' --wait 1000 \
...
```
```
node .cline/skills/pw-playwright-fieldkit/scripts/flow.mjs flow.json --out report/debug
```
