...
`, "https://example.test", "PlaywrightFieldKit");
  ...
  const workDir = mkdtempSync(join(tmpdir(), "playwright-fieldkit-test-"));
  let passed = false;
  try {
    await listen(server);
    ...
    const rule = readFileSync(resolve(WORKSPACE_ROOT, ".clinerules", "playwright-fieldkit.md"), "utf8");
    ...
    const debugWorkflow = readFileSync(resolve(SKILL_DIR, "references", "workflows", "debug-site.md"), "utf8");
    const recordWorkflow = readFileSync(resolve(SKILL_DIR, "references", "workflows", "record-flow.md"), "utf8");
    assert.match(rule, /activate the\s+`playwright-fieldkit` skill/, "always-on rule omitted skill activation");
