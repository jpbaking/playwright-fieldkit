# /explore-site — map a website end to end
...
`(cd .cline/skills/playwright-fieldkit/scripts && npm install && npx playwright install chromium)` and stop until done.

## Step 2 — Crawl
```
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <START_URL> --depth 2 --max-pages 40 --out report/explore
```
...
- If the interesting pages are behind a login, first run `/explore-site` after
...
```
node .cline/skills/playwright-fieldkit/scripts/inspect.mjs <THAT_URL> --wait 1500 --out report/inspect
```
...
- **Recommended next move:** e.g. "run /generate-tests on the login form" or
  "run /debug-site on the 500 at /account".
