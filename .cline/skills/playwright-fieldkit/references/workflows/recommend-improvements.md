# /recommend-improvements — audit a site and suggest fixes
...
```
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <START_URL> --depth 2 --max-pages 40 --out report/audit
```
...
- Admin/gated routes reachable without auth (from /discover-features).
...
matters**. Group by axis, ordered by priority. Offer to `/generate-tests` for the
flows you'd want protected against regression, and `/debug-site` for anything
