# /compare-runs — diff two crawls to catch regressions and changes
...
```
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <URL> --depth 2 --max-pages 40 --out report/before
# ...deploy / make the change...
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <URL> --depth 2 --max-pages 40 --out report/after
```
**Logged-out vs logged-in / role vs role (feature discovery):**
```
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <URL> --out report/anon
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <URL> --storage-state auth.json --out report/auth
```
...
- **Regression check:** lead with removed pages, status regressions, and new
  errors — those are likely breakage. Offer to `/debug-site` each one.
- **Feature discovery:** lead with new pages and new forms — those are the gated
  or role-specific features. Offer to `/discover-features` or `/generate-tests`.
