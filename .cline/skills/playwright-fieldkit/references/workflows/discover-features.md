# /discover-features — find undocumented and gated features
...
```
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <START_URL> --depth 3 --max-pages 60 --out report/discover
```
...
```
node .cline/skills/playwright-fieldkit/scripts/save-auth.mjs <LOGIN_URL> --headed --out auth.json      # log in once
node .cline/skills/playwright-fieldkit/scripts/crawl.mjs <START_URL> --depth 3 --max-pages 60 \
     --storage-state auth.json --out report/discover-auth
```
Then **diff the two crawls automatically** — don't compare by hand:
```
node .cline/skills/playwright-fieldkit/scripts/compare.mjs report/discover report/discover-auth --out report/gated
```
...
```
node .cline/skills/playwright-fieldkit/scripts/inspect.mjs <HIDDEN_URL> --storage-state auth.json --wait 1500
```
