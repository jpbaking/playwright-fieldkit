...
| Installed rule adapters (`.agents/rules/`, `.claude/rules/`, `.clinerules/`) | Always-on activation hint | Same body, host-specific discovery path |
| Generated `.clinerules/workflows/pw-*.md` | Cline slash-command shortcuts | Cline-specific, regenerated by the installer |

The `.agents/skills` location is the shared project convention for Codex,
Antigravity, and current Cline, and it is the single runtime home of the
scripts (the byte-identical `.claude/skills/` copy exists for Claude Code's
skill discovery and routes script execution to the `.agents` copy). Keep the
package there: the skill and examples intentionally use stable workspace-root
commands such as:
