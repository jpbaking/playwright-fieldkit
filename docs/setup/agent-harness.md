...
Playwright FieldKit is packaged as a portable Agent Skill for Codex, Claude
Code, Google Antigravity, and Cline; its browser runtime, templates, and
workflow playbooks call no harness APIs. Any coding-agent harness that can run
shell commands and read workspace files can use it.

## What is portable

| Part | Purpose | Harness dependency |
|---|---|---|
| `~/.agents/skills/pw-playwright-fieldkit/scripts/` | Deterministic browser and QE commands | None; ordinary Node.js |
| `~/.agents/skills/pw-playwright-fieldkit/templates/` | Flows, charters, matrices, and test scaffolds | None |
| `~/.agents/skills/pw-playwright-fieldkit/references/workflows/` | Canonical operating playbooks | None; Markdown |
| `~/.agents/skills/pw-playwright-fieldkit/SKILL.md` | Intent router and safety contract | Skill-aware agents can load it directly; others can read it as instructions |
| Installed rule adapters (global rules locations, or pointer blocks in `~/.codex/AGENTS.md` / `~/.claude/CLAUDE.md`) | Always-on activation hint | Same body, host-specific discovery path |
| Generated global Cline `pw-*.md` workflow stubs | Cline slash-command shortcuts | Cline-specific, regenerated by the install |

`~/.agents/skills/` is Codex's user-global skills location and the single
runtime home of the scripts (the byte-identical copies in the other
harnesses' global skill directories exist for discovery and route script
execution back to it). Keep the package there: the skill and examples
intentionally use stable home-anchored commands such as:

```bash
node ~/.agents/skills/pw-playwright-fieldkit/scripts/crawl.mjs <URL>
```
...
application, read `~/.agents/skills/pw-playwright-fieldkit/SKILL.md`. Select and follow
...
```markdown
Read and follow `~/.agents/skills/pw-playwright-fieldkit/SKILL.md`. Treat its linked
...
> Read `~/.agents/skills/pw-playwright-fieldkit/SKILL.md`, follow the workflow matching
