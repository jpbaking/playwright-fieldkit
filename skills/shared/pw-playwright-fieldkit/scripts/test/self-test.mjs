...
`, "https://example.test", "PlaywrightFieldKit");
  ...
  try {
    ...
    const rule = readFileSync(resolve(WORKSPACE_ROOT, "rules", "shared", "pw-playwright-fieldkit.md"), "utf8");
    ...
    // Cline slash-command shortcuts are generated by the installer, not
    // committed; verify every canonical playbook is referenced by SKILL.md
    // so implicit routing (and the generated shortcuts) stay one-to-one.
    const canonicalNames = readdirSync(canonicalWorkflowDir).filter((name) => name.endsWith(".md")).sort();
    assert(canonicalNames.length > 0, "no canonical workflow playbooks found");
    for (const name of canonicalNames) {
      assert(skillRouter.includes(`references/workflows/${name}`), `SKILL.md does not route to canonical workflow ${name}`);
    }
    ...
    console.log(`✓ packaging: activation rule, skill router, and ${canonicalNames.length} canonical workflows`);
